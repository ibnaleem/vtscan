...
import (
	...
	"github.com/ibnaleem/vtscan/internal/hash"
...
var fileCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, arg := range args {
			if hash.CheckHash(arg) {
				body, statusCode, err := c.Get(fmt.Sprintf("files/%s", arg))
				if err != nil {
					return err
				}
				if statusCode != 200 {
				...
				printer.FileResponse(fileResponse)
				return nil
			}
			...
			h, err := hash.CalculateFileSHA256(arg)
			if err != nil || h == "" {
				return fmt.Errorf("vtscan: could not hash %s: %v", arg, err)
			}

			body, statusCode, err := c.Get(fmt.Sprintf("files/%s", h))
			if err != nil {
				return err
			}
			if statusCode != 200 {
